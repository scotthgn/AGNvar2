...
class AGNsed_CL_gammaStatic(AGNobject):
    ...
    def __init__(self,
        ...
        cos_inc : float
            ...
            Defualt : 0.9
        kTe_hot : float
            Hot Compton electron temperautre. Sets the high energy roll-over 
            for the hot Compton region - Units : keV
            Default : 100
        kTe_warm : float
            ...
            Default : 0.2
        gamma_hot : float
            Spectral index for the hot Comptonisation region. Remains fixed
            throughout
            Default : 1.7
        gamma_warm : float
    ...
    def evolve_intrinsicSED(self, reverberate=True, reverb_only=False,
        ...
        if len(self.logr_hc_bins) > 1:
            Lhot_var = self.sed.hot_spec(xt_hot, gamma=self.gamma_h, kte=self.kTh,
        ...
        self.parvar_dict = {'kTseed_hot':ktseeds,
                            'Lseed_hot':Lseed,
                            'Ldiss_hot':Ldiss}
